...
void TypSetPwr::initViewBar()
{
    ...
    pname << tr("参数");
    QStringList parms;
    parms << tr("测试延时(s)")<< tr("Vcc补偿(V)") << tr("Vsp补偿(V)")
...
void TypSetPwr::initSettings()
{
    ...
    for (int i=0; i < iMode->rowCount()*2; i++) {
        QString str = tmpSet.value(addr + CACHEPWR*row + i).toString();
        if (i == 8)
...
void TypSetPwr::saveSettings()
{
    ...
    for (int i=0; i < iMode->rowCount()*2; i++) {
        ...
        tmpMsg.insert(addr + CACHEPWR*0x00 + i, str);
    }
    for (int i=0; i < vMode->rowCount(); i++) {
        QString str = QString::number(vMode->index(i, 0).data().toDouble());
        tmpMsg.insert(addr + CACHEPWR*0x01 + i, str);
    }
    for (int i=0; i < pMode->rowCount(); i++) {
        ...
        if (i == 5)
            str = QString::number(turns.indexOf(str));
        tmpMsg.insert(addr + CACHEPWR*0x02 + i, str);
    }
    for (int i=0; i < tMode->columnCount(); i++) {
        QString str = tMode->index(0, i).data().toString();
        tmpMsg.insert(addr + CACHEPWR*0x03 + i, str);
    }
...
void TypSetPwr::recvShowEvent()
{
    ...
    tmpMap.insert("text", QString("6004 NOLOAD"));
