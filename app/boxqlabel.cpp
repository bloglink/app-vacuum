...
void BoxQLabel::setText(QString text, int mode)
{
    ...
    case 2:
        str3 = text;
        stop = qMax(200, text.toInt());
        stop = qMin(370, text.toInt());
        break;
    case 3:
        str4 = text;
        from = qMin(200, text.toInt());
        from = qMax(30, text.toInt());
        break;
    default:
        break;
    }
...
void BoxQLabel::drawImpWave(QPainter *painter)
{
    ...
    painter->setPen(QPen((isok > 0) ? Qt::red : Qt::green, 1, Qt::SolidLine));
...
void BoxQLabel::drawAllText(QPainter *painter)
{
    ...
    int w = this->width();
    int h = this->height();
    painter->setPen(QPen(this->isEnabled() ? Qt::white : Qt::gray));
    if (!str1.isEmpty()) {
        int x = 2;
...
void BoxQLabel::mousePressEvent(QMouseEvent *e)
{
    if (mode == 4) {
        int w = this->width();
        if (e->x() > w/2) {
            stop = qMin(370, e->x() * IMP_SIZE / w);
        } else {
            from = qMax(30, e->x() * IMP_SIZE / w);
        }
        this->update();
    }
    emit clicked();
